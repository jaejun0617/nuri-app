...
## Chapter X. 홈 UI 1차 업그레이드 — 프로필 HERO 카드(스샷 레이아웃) 완성

홈(LoggedInHome) 상단의 프로필 HERO 카드를 스샷 레이아웃 기준으로 1차 완성했다.

### ✅ 적용 내용

- **중앙 정렬 프로필 카드 레이아웃**으로 UI 교체
- **보라색 ‘함께한 시간’ Pill(음영 포함)** 적용
- **이름 / 견종 / 나이 / 성별 / 몸무게** + **생년월일** 표기 추가
- **아코디언 섹션** 구현: `취미 / 좋아하는 것 / 싫어하는 것 / #태그`
  - 개별 펼치기/접기
  - **‘모두 펼치기 / 모두 접기’** 전체 토글 제공
- **상태/훅 안정성**
  - `recordStore`는 `byPetId[petId]` 직접 구독
  - fallback은 **동일 참조(FALLBACK_RECORDS_STATE)**로 유지
  - `activePetId` 변경 시 **아코디언/최근기록 확장 상태 초기화**로 UX 안정화

### 다음 스텝 트리거

**“홈 UI 2차 업그레이드 — 카드 애니메이션 + 퍼포먼스 튜닝 들어가자.”**

## Chapter X. Home UI 1차 리디자인 (Purple Theme + Profile Accordion)

- 로그인 홈(LoggedInHome) 상단 프로필 영역을 **흰색+보라색 브랜드 톤**으로 통일했다.
- HERO(프로필 카드)는 과한 “떠있는 카드” 느낌을 제거하고 **Flat + Border** 기반으로 정리했다.
- 프로필 아바타는 **퍼플 glow/shadow + white ring**로 감성 포인트를 강화했다.
- 핵심 정보(이름/견종/나이/성별/몸무게/생년월일) + **함께한 시간 Pill**을 상단에서 한 번에 전달한다.
- 하단 정보는 아코디언으로 정리:
  - **모두펼치기** + 개별 펼치기(취미/좋아하는 것/싫어하는 것/#태그)
- “오늘의 메시지”는 **보라 tint + shadow**로 카드 아래 음영을 추가해 섹션 구분을 명확히 했다.
- “누리와의 소중한 기록” 섹션을 시작하며,
  - “오늘날의 사진”은 **보라 tint overlay**로 톤을 통일하고 “전체보기”를 제공한다.
- Hook/스토어 안정성:
