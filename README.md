...
- 상단 프로필 카드를 아바타 + 이름 + `프로필 수정` 구조로 다시 구성했다.
  - 스크린샷 톤에 맞춰 여백, 카드 반경, 배지 위치를 통일했다.
- 계정 섹션에 `내 정보 수정`, `비밀번호 변경`, `로그아웃`, `회원탈퇴`를 넣었다.
- `내 정보 수정` 모달에서 닉네임을 바꿀 수 있게 했고, 닉네임은 월 1회만 변경되도록 로컬 정책을 분리했다.
...
- 프로필 카드와 메뉴 행 간격이 일정해서 전체 정보 구조가 더 명확해졌다.
