...
## Chapter X. Home 감성 강화 v1 (오늘의 추억 + 시간대 메시지)

### 목표

- 홈 화면을 CRUD 중심에서 “서비스의 얼굴”로 전환
- KST 기준으로 오늘 컨텍스트를 안정적으로 제공
- 화면 구성이 단순하지만 ‘앱 같다’는 느낌을 만들기

### 변경 사항

- `src/utils/date.ts` 추가

  - KST 기준 날짜/시간 유틸 통합
  - 함께한 시간(D-day) 계산 및 시간대(morning/noon/evening) 판별

- `src/services/home/homeRecall.ts` 추가

  - 오늘의 추억 선택 로직(클라이언트 캐시 버전)
  - 우선순위:
    1. 작년 오늘(월/일 동일) occurredAt 기록 우선
    2. 없으면 전체 기록 중 랜덤
  - 하루 1회 고정(AsyncStorage)

- `LoggedInHome` 개선
  - 오늘의 메시지 섹션: 기존 3개 썸네일 → 단일 감성 카드로 통일
  - 카드 탭 시: 선택된 “오늘의 추억” 상세로 이동(없으면 기록하기로 유도)
  - LoggedInHome 스타일을 독립 파일로 분리하여 수정 범위 안정화

### 다음 챕터 트리거

> 이제 클라이언트 캐시(AsyncStorage) 대신, Supabase `daily_recall` 테이블을 사용해 “서버 고정 리콜(하루 1회)”로 전환한다.  
> (여러 기기 동기화 + AI 메시지/구독 확장 기반)

## Chapter X. Home 감성 강화 v2 (메시지/오늘사진 분리 + 최근기록 주간 7개)

### 목표

- 홈을 CRUD 화면이 아니라 “서비스의 얼굴”로 보이게 만들기
- 오늘 컨텍스트(날짜/요일/오늘의 메시지/오늘의 사진)를 분리해 감성 강화
- 최근 기록을 ‘앨범/피드’처럼 읽히게 구성

### 구현 내용

- `src/utils/date.ts`

  - KST 기준 날짜/요일 포맷(`formatKstDateWithWeekday`)
  - 기록 날짜 기준 daysAgo 계산(`daysAgoFromKstToday`) → 주간 버킷용

- `src/services/home/homeRecall.ts`

  - 오늘날의 사진 픽(하루 1회 캐시)
  - 작년 오늘(월/일 동일) 사진 우선, 없으면 랜덤(이미지 있는 기록 중)

- `LoggedInHome`
  - “오늘의 메시지” 섹션 분리 + 날짜/요일 표시
  - “오늘날의 사진” 섹션 분리 + 작년 오늘 우선/랜덤
  - “최근 기록”:
    - 유튜브 썸네일 스타일(썸네일 + 제목 + 내용 + 날짜)
    - 1주 간격으로 최대 7개 노출(주간 버킷)
    - 하단 “기록하기” 버튼
    - 7개 채워지면 “더보기” 노출 → 추억보기(TimelineTab) 이동
