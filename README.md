...
- 산책 위치 기반 기능은 `선택 동 실제 조회 기준`, `조회 anchor 기준 거리`, `상세 연관 장소 더보기` 구조로 보강되었습니다
- 산책 위치 기반 기능은 `현재 GPS 기반 주변 추천 + 직접 검색`, `현재 위치 기준 거리`, `검색 입력 화면 키보드/스크롤 안정성` 기준으로 단순화되었습니다
- 펫동반 장소 탐색은 `현재 GPS 기반 주변 추천 + 직접 검색`, `리스트/상세/외부 지도 연결`, `현재 데이터는 Kakao Local 외부 검색 결과` 기준으로 시작되었습니다
...
- 산책과 펫동반 장소 탐색은 모두 `현재 GPS 기반 주변 추천 + 직접 검색` 정책을 공유합니다.
- 거리 기준은 항상 현재 위치 기준이며, 현재 위치가 없으면 거리값은 null로 유지합니다.
- 펫동반 장소 탐색은 현재 `Kakao Local 외부 검색`이 source of truth이며, Supabase 장소 DB는 아직 없습니다.
- 펫동반 결과는 `외부 후보`와 `서비스 검증 상태`를 분리해 노출하며, 카카오 응답만으로 확정 표시하지 않습니다.
- 상세 화면에서 지도 미리보기, 외부 지도 열기, 현재 결과 컬렉션 기반 연관 장소 더보기를 제공합니다.
...
- 위치 기반 기능 챕터 시작
  - 전체메뉴에서 `우리동네 산책 / 산책 동선 추천`, `펫동반 음식점 찾기`를 각각 별도 메뉴/별도 화면으로 분리
  - GPS 현재 위치 + 직접 검색 + 장소 상세 + 지도 열기 구조 시작
  - 현재 데이터 전략은 `외부 API 결과 + 향후 Supabase 메타 혼합` 방향으로 유지
