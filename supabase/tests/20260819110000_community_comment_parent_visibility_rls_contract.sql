...
begin
  if to_regclass('public.comments') is null then
    raise exception 'community_comments_table_missing';
  end if;
  ...
  if v_helper_oid is null then
    raise exception 'community_parent_visibility_helper_missing';
  end if;
  ...
  if not exists (
    ...
    raise exception 'community_parent_visibility_helper_security_mismatch';
  end if;
  ...
  or has_function_privilege(
    ...
    raise exception 'community_parent_visibility_helper_direct_execute_granted';
  end if;
  ...
  if v_select_policy_qual is null
    or lower(v_select_policy_qual) not like '%community_parent_post_visible_to_current_user%'
    or lower(v_select_policy_qual) not like '%status = ''active''%'
    or lower(v_select_policy_qual) not like '%deleted_at is null%'
    or lower(v_select_policy_qual) like '%user_id = auth.uid()%'
    or lower(v_select_policy_qual) like '%is_community_admin()%' then
  ...
  if not exists (
    ...
    raise exception 'community_blocks_rls_regressed';
  end if;

  -- Read-only smoke for the same helper used by the direct-table policy.
  v_helper_result := public.community_parent_post_visible_to_current_user(
  ...
  -- A current public row exercises the positive helper path without exposing
  -- post or comment content and without creating a production fixture.
  select p.id
  ...
  if v_public_post_id is not null
    and not public.community_parent_post_visible_to_current_user(v_public_post_id) then
