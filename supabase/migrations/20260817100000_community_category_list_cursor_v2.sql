...
-- NURI-09: additive category-aware community list contract.
-- The established v1 RPC and its cursor v2 contract remain unchanged so an
-- older client can continue to read the primary all/popular/notice feeds.

create index if not exists idx_posts_active_public_category_all_notice_first
  on public.posts (
    category,
    is_notice desc,
...
create or replace function public.community_list_posts_v2(
...
begin
  ...
  if p_cursor is not null then
    ...
    if v_cursor_parse_failed or v_cursor_version is distinct from 3 then
      perform public.raise_community_notice_error(
  ...
  if v_filter = 'popular' then
    ...
    v_next_cursor_sql := 'jsonb_build_object(''version'', 3, ''filter'', ''popular'', ''category'', $3, ''pageSize'', $1, ''likeCount'', (last_visible.post_row).like_count, ''commentCount'', (last_visible.post_row).comment_count, ''createdAt'', (last_visible.post_row).created_at, ''id'', (last_visible.post_row).id)';
  elsif v_filter = 'notice' then
    ...
    v_next_cursor_sql := 'jsonb_build_object(''version'', 3, ''filter'', ''notice'', ''category'', $3, ''pageSize'', $1, ''noticePublishedAt'', (last_visible.post_row).notice_published_at, ''createdAt'', (last_visible.post_row).created_at, ''id'', (last_visible.post_row).id)';
  else
    ...
    v_next_cursor_sql := 'jsonb_build_object(''version'', 3, ''filter'', ''all'', ''category'', $3, ''pageSize'', $1, ''isNotice'', (last_visible.post_row).is_notice, ''noticePublishedAt'', (last_visible.post_row).notice_published_at, ''createdAt'', (last_visible.post_row).created_at, ''id'', (last_visible.post_row).id)';
  end if;
...
revoke all on function public.community_list_posts_v2(text, text, integer, jsonb)
  from public, anon, authenticated;
grant execute on function public.community_list_posts_v2(text, text, integer, jsonb)
