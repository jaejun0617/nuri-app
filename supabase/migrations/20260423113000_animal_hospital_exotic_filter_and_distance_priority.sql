...
comment on table public.animal_hospital_verifications is
  '동물병원 field-level 검수 기록. phone/coordinates/thumbnail/open24Hours/exoticAnimalCare approved 값만 앱 verified gate 입력으로 소비한다.';
...
returns table (
  ...
  pending_exotic_animal_care bigint,
  ...
  approved_exotic_animal_care_coverage bigint,
...
begin
  ...
  approved_counts as (
    select
      ...
      count(distinct animal_hospital_id) filter (where field_key = 'exoticAnimalCare') as exotic_count
  ...
  pending_counts as (
    select
      ...
      count(*) filter (where field_key = 'exoticAnimalCare') as exotic_count
    from public.animal_hospital_verifications
  ...
  select
    ...
    coalesce((select exotic_count from pending_counts), 0),
    ...
    coalesce((select exotic_count from approved_counts), 0),
...
drop function if exists public.animal_hospital_ops_review_items(text, text, text, text, integer);

create function public.animal_hospital_ops_review_items(
...
begin
  ...
  select
    ...
    v.status,
    case
      when v.field_key in ('phone', 'coordinates', 'thumbnail', 'open24Hours', 'exoticAnimalCare') then coalesce((
        select av.verified_value
        from public.animal_hospital_verifications av
        where av.animal_hospital_id = h.id
          and av.field_key = v.field_key
