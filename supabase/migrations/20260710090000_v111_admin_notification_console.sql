...
revoke all on function public.admin_send_qa_user_notification_v1(text, text, text, text, jsonb)
  from public, anon, authenticated;
revoke all on function public.admin_notification_audit_feed_v1(integer)
  from public, anon, authenticated;

grant execute on function public.admin_send_qa_user_notification_v1(text, text, text, text, jsonb)
  to authenticated;
grant execute on function public.admin_notification_audit_feed_v1(integer)
  to authenticated;

comment on function public.admin_send_qa_user_notification_v1(text, text, text, text, jsonb)
  is 'Admin-only QA-target notification sender for the separated admin console. It hides raw user ids from the console, forbids broadcast, and excludes pending-deletion profiles.';
comment on function public.admin_notification_audit_feed_v1(integer)
  is 'Admin-only audit feed with redacted target labels for the separated notification management console.';
