-- 실행 migration mirror: docs/sql/반려동물과-여행/반려동물과여행-트러스트레이어-최종.sql
-- 이 파일은 반려동물과 여행 Trust Layer의 현재 최종 설계본이다.
-- `pet_travel_places`, `pet_travel_pet_policies`, `pet_travel_user_reports`와 공개 trust 판단용 객체를 관리한다.
-- 문서용 source of truth이지만, linked remote 적용 여부는 별도로 확인해야 한다.
-- 현재 linked remote에 대표 테이블이 없으면 local-only 최종본으로 보고, 이 파일을 기준으로 적용 순서와 drift를 분석한다.
