...
-- NURI-09: keep the public community feed notice-first at the server boundary.
-- This migration is additive. It does not rewrite existing content or weaken
-- the notice RLS/operator contract.
do $$
begin
  ...
  if not exists (
    ...
    raise exception 'community_notice_columns_missing'
      using errcode = '42703';
  end if;
end
$$;

create index if not exists idx_posts_active_public_all_notice_first
  on public.posts (
    is_notice desc,
...
begin
  ...
  if p_cursor is not null then
    ...
    -- Cursor versioning prevents an old created_at/id cursor from being
    -- interpreted against the new notice-first ordering.
    ...
    if v_cursor_parse_failed or v_cursor_version is distinct from 2 then
      perform public.raise_community_notice_error(
        'community_cursor_version_unsupported',
        '22023'
      );
    ...
    if v_cursor_parse_failed
      ...
      or (v_filter = 'popular' and (
        v_cursor_like_count is null
        or v_cursor_comment_count is null
      ))
  ...
  if v_filter = 'popular' then
    ...
    v_next_cursor_sql := 'jsonb_build_object(''version'', 2, ''filter'', ''popular'', ''pageSize'', $1, ''likeCount'', (last_visible.post_row).like_count, ''commentCount'', (last_visible.post_row).comment_count, ''createdAt'', (last_visible.post_row).created_at, ''id'', (last_visible.post_row).id)';
  elsif v_filter = 'notice' then
    ...
    v_next_cursor_sql := 'jsonb_build_object(''version'', 2, ''filter'', ''notice'', ''pageSize'', $1, ''noticePublishedAt'', (last_visible.post_row).notice_published_at, ''createdAt'', (last_visible.post_row).created_at, ''id'', (last_visible.post_row).id)';
  else
    -- Descending NULLS LAST semantics are expanded explicitly so the keyset
    -- predicate remains identical to the ORDER BY at the notice/normal edge.
    ...
    v_next_cursor_sql := 'jsonb_build_object(''version'', 2, ''filter'', ''all'', ''pageSize'', $1, ''isNotice'', (last_visible.post_row).is_notice, ''noticePublishedAt'', (last_visible.post_row).notice_published_at, ''createdAt'', (last_visible.post_row).created_at, ''id'', (last_visible.post_row).id)';
  end if;
