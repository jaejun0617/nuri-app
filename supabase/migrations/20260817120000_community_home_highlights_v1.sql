...
-- NURI-09: additive Home highlights read contract.
-- Home's existing policy includes notices when they satisfy the public/active
-- read contract. This RPC therefore does not add a notice exclusion predicate.

create index if not exists idx_posts_active_public_home_highlights_v1
  on public.posts (
    like_count desc,
    created_at desc,
    id desc
...
create index if not exists idx_posts_active_public_category_home_highlights_v1
  on public.posts (
    category,
    like_count desc,
    created_at desc,
    ...
    select
      ...
      (
        select jsonb_build_object(
          'version', 1,
          'category', (last_visible.post_row).category,
          'pageSize', $1,
