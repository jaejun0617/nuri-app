...
-- NURI-09 / COMMUNITY-POLICY-001:
-- Store directional user blocks while applying mutual exclusion only inside
-- authenticated Home and Community list RPCs. Incoming relations are never
-- exposed through the client-facing block-table SELECT policy.
create table public.community_user_blocks (
  blocker_user_id uuid not null references auth.users(id) on delete cascade,
  blocked_user_id uuid not null references auth.users(id) on delete cascade,
  created_at timestamptz not null default timezone('utc', now()),
  constraint community_user_blocks_pkey
    primary key (blocker_user_id, blocked_user_id),
  constraint community_user_blocks_no_self_block
    check (blocker_user_id <> blocked_user_id)
);

create index community_user_blocks_blocked_blocker_idx
...
revoke all on table public.community_user_blocks from public, anon, authenticated;
grant select, insert, delete on table public.community_user_blocks to authenticated;

create policy community_user_blocks_select_own_outgoing
  on public.community_user_blocks
  for select
  to authenticated
  using ((select auth.uid()) = blocker_user_id);

create policy community_user_blocks_insert_own_outgoing
  on public.community_user_blocks
  for insert
  to authenticated
  with check (
...
create policy community_user_blocks_delete_own_outgoing
  on public.community_user_blocks
  for delete
  to authenticated
  using ((select auth.uid()) = blocker_user_id);
...
begin
  ...
  if v_limit not in (3, 30, 50, 100, 150, 200) then
    perform public.raise_community_notice_error('community_page_size_invalid', '22023');
  end if;
