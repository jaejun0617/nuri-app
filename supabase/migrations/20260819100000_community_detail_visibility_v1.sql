...
-- NURI-09 / COMMUNITY-DETAIL-001:
-- Expose the existing public Community detail projection through a guarded
-- read RPC. The caller identity comes only from auth.uid(); unavailable,
-- moderated, deleted, and mutually blocked posts intentionally share the
-- same {item: null} response so the reason cannot be inferred.
