...
function makePost(id: string, likeCount: number): CommunityPost {
  return {
    id,
    authorId: `author-${id}`,
...
describe('home community highlights service', () => {
  ...
  it('uses popular/all with the minimum bounded RPC page and preserves server order', async () => {
    ...
    const result = await fetchHomeCommunityHighlights();
  ...
  it('deduplicates concurrent requests and serves a fresh cache without refetching', async () => {
    ...
    const firstRequest = fetchHomeCommunityHighlights();
    const secondRequest = fetchHomeCommunityHighlights();
    ...
    await fetchHomeCommunityHighlights();
