...
이 문서는 도메인별 운영자(Admin) UI 실계정 QA를 v1.1 완료 후 최종 스토어 제출 전 Gate에서 일괄 처리하기 위한 parking backlog다.
...
- Admin QA는 v1.0 기능 blocker가 아니며, v1.1 완료 후 최종 스토어 제출 전 Admin QA batch에서 처리한다.
...
| --- | --- | --- | --- | --- |
| ADMIN-QA-001 | 우리동네 동물병원 | 운영자 검수 UI approve/reject/held 실계정 QA | Parking. 2026-04-29 PO Lock-in에 따라 동물병원은 유저 서비스 기준 100% 완료로 close한다. | v1.1 완료 후 최종 스토어 제출 전 Admin QA batch |
