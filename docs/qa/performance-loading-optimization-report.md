...
## 2026-07-11 production readiness 추가 최적화

PO 결정에 따라 Play Store 자산 준비와 앱 폰트/디자인 전체 리뉴얼은 아직 진행하지 않고, 앱 자체의 실서비스급 로딩/안정화만 추가로 보강했다.

| 영역 | 추가 적용 | 결과 |
| --- | --- | --- |
| 기록/일정 disk cache | `src/services/local/homeRecordScheduleCache.ts`를 추가해 Home 기록/일정 preview를 `userId + petId` scope로 저장/복원한다. | cache hit 시 Home 카드가 remote bootstrap 전 먼저 채워진다. |
| cache 보안 | schema version, 3일 TTL, corrupt fallback, pet/user scope validation, logout/account transition clear를 적용했다. | 다른 사용자/펫 cache 노출 방지 |
| dashboard summary RPC | `get_user_activity_long_summary_v1`를 활동·칭호 service에서 우선 사용한다. | level summary와 community/comment count 중복 query 감소 |
| Ranking React Query | `누리 랭킹`을 category별 query key/stale/cache policy로 전환했다. | 탭별 cache와 skeleton 안정화 |
| Timeline/Community | 첫 렌더와 무관한 count/summary/post fetch를 interaction 이후로 지연했다. | 리스트 shell과 첫 paint 경쟁 완화 |

세부 보고서는 `docs/qa/production-readiness-optimization-report.md`, `docs/qa/android-navigation-keyboard-back-qa-report.md`, `docs/qa/cache-security-audit-report.md`를 source of truth로 둔다.

## 병목 분석

| 구분 | 확인 내용 | 판정 |
| --- | --- | --- |
| Home initial blocking query | AppProviders는 프로필/펫을 병렬 fetch하고 pet cache를 먼저 hydrate한다. Home 자체는 shell 렌더 후 기록/일정, 날씨, 추천 가이드, 홈 칭호 조회가 같은 시점에 시작된다. 2026-07-11 추가 최적화로 기록/일정 preview도 disk cache를 먼저 주입한다. | Home shell 유지, cache hit 시 record/schedule 카드 blank 감소 |
...
| 랭킹 tab loading | 탭 전환 시 이전 탭 row가 새 탭 label 아래 잠시 남을 수 있고, 탭별 cache가 없었다. 2026-07-11 추가 최적화로 React Query category key 기반 cache로 전환했다. | 탭별 cache와 skeleton 적용 |
...
| Ranking | `누리 랭킹`은 React Query의 탭별 query key/cache/stale policy를 사용한다. 방문한 탭은 cached rows를 재표시하고, 새 탭은 skeleton row를 표시한다. |
| Cache security | Home title badge cache key와 Home record/schedule cache key는 모두 `userId + petId` scoped다. title badge stale cache는 7일, record/schedule stale cache는 3일 후 제거한다. |
...
| cached data 우선 표시 | Home 대표 칭호 badge cache, Home 기록/일정 preview cache 적용 |
...
| 랭킹 lazy/cache | selected tab만 fetch, React Query 탭별 cache 유지 |
| DB/RPC/RLS 변경 | 없음 |
...
| 후보 | 분류 | 이유 |
| --- | --- | --- |
| 기록/일정 store disk cache | 완료 | Home preview 기준 user/pet scoped disk cache를 적용했다. |
| Activity dashboard summary RPC 전면 사용 | 부분 완료 / V1.2 고도화 | level summary와 community/comment count는 long summary RPC를 우선 사용한다. pet별 상세 count/streak는 기존 분리 query 유지. |
| Ranking React Query 전환 | 완료 | category별 query key/stale/cache policy로 전환했다. |
