...
| COMMUNITY-POLICY-001 | P1 | block 관계와 caller 기준 feed visibility 계약이 없었고, Home/List RPC에 blocked-user predicate가 없었다. Master가 authenticated feed 상호 비노출 정책을 결정했으며 backend 계약 구현이 남아 있다. | NURI-09-Supabase·RLS·RPC·운영DB / NURI-06-커뮤니티·모더레이션 | RELEASE_BLOCKING / POLICY_APPROVED_BACKEND_PENDING | block relation·RLS·visibility helper·Home/List RPC predicate·negative contract test를 additive 변경으로 닫고 row-level evidence 확보 |
