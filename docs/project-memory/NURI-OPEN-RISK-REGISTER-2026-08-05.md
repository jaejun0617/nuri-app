...
| COMMUNITY-POLICY-001 | P1 | block 관계·RLS·Home/List mutual feed predicate backend는 구현됐지만 실제 authenticated block action과 controlled negative evidence가 아직 없다. | NURI-09-Supabase·RLS·RPC·운영DB / NURI-06-커뮤니티·모더레이션 | RELEASE_BLOCKING / BACKEND_COMPLETE_CONTROLLED_QA_PENDING | 앱 block action integration 후 A/B mutual exclusion, unblock 재노출, unrelated user, anonymous behavior를 controlled QA로 확인 |
...
| COMMUNITY-BLOCK-ACTION-001 | P1 | `community_user_blocks` backend는 존재하지만 현재 앱에 block action UI/service가 없어 사용자가 정상 경로로 block/unblock을 실행할 수 없다. | NURI-06-커뮤니티·모더레이션 | RELEASE_BLOCKING / APP_INTEGRATION_PENDING | 기존 Community action surface에 최소 block/unblock service와 UI를 연결하고 authenticated QA 경로를 확보 |
