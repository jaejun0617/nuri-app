...
4. `NURI-09-Supabase·RLS·RPC·운영DB`: AUTH-001의 remote Provider read-only 검증과 policy/RPC/grant 증거를 지원한다. Naver 정책은 Google ON/Kakao ON/Naver 완전 제거/Apple OFF로 이미 최종 확정됐다.

## Priority 2

5. `NURI-01-인증·온보딩`: AUTH-001에 따라 Naver 사용자 노출·route·flow·helper·config·env·dependency·current 문서 잔존을 제거하고 Google/Kakao 회귀 기준을 고정한다. 순서는 NURI-01 → NURI-09 → NURI-12다.
