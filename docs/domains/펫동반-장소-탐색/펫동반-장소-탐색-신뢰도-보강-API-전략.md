...
2026-03-19 기준으로 실제로 의미 있는 것은 아래다.
...
3. `pet_place_source_links.provider + provider_place_id` 또는 내부 매칭 규칙 기준으로 canonical meta 연결
4. Supabase의 `verification_status`, `source_type`, `pet_policy_text`를 최우선으로 반영
5. 외부 신호는 `검증 근거` 또는 `검수 우선순위`로만 사용

즉 canonical truth는 `pet_place_service_meta`, 외부 매핑은 `pet_place_source_links`, 보조 신호는 `pet_place_external_signals`로 분리하는 것이 맞다.
...
## 13. 반려동물과 여행 도메인과의 연결

일상 생활권 탐색과 여행/관광 탐색은 데이터 소스가 다르므로 장기적으로 별도 메뉴가 맞다.

- `펫동반 장소 탐색`
  - Kakao Local 중심
  - 현재 위치 기반 빠른 탐색
- `반려동물과 여행`
  - 관광공사 반려동물 동반여행 서비스 중심
  - 여행지 / 숙소 / 문화시설 / 야외 공간

여행 도메인은 런타임 직접 호출보다 배치 적재 + Supabase 메타 merge가 더 적합하다.
