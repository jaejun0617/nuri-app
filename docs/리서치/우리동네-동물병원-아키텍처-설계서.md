...
## 2026-06-30 전국 커버리지 / coordinate missing read-only audit

- 판정: `우리동네 병원 찾기 전국 확장 완료, coordinate missing 122건은 release blocker 아님`
- 이번 턴 작업 성격: 신규 seed/DB write 없이 public projection과 대표 좌표를 read-only로 확인했다.
- public active count: 5,427건
- 기존 ingest/canonical evidence: 약 10,507건. 이번 턴에는 로컬 `psql` 부재로 SQL catalog 직접 재확인은 하지 못했으므로 기존 evidence와 public projection을 분리해 기록한다.
- public active 시도/광역 분포: 경기 1,398, 서울 969, 경북 365, 경남 364, 충남 297, 부산 280, 인천 253, 전남 240, 전북 220, 대구 213, 충북 180, 강원 167, 광주 129, 대전 121, 제주 116, 울산 80, 세종 35.
- 대표 좌표 smoke: 일산, 서울, 경기, 인천, 부산, 대구, 대전, 광주, 울산, 세종, 제주, 강원, 충청, 전라, 경상 모두 10km/20건 반환.
- public active address missing: 0건
- public active coordinate missing: 122건.
- coordinate missing 122건 세부: primary address 122건, road address 110건, lot address 121건, official phone 82건, providerPlaceUrl 0건.
- UX 판정: nearby 좌표 기반 리스트는 좌표가 있는 병원만 반환한다. 좌표 없는 병원은 text search/detail에서 주소와 전화번호가 있으면 정보형으로 표시하고, 좌표 기반 지도/길찾기 CTA는 숨기거나 주소 기준 안내로 처리한다.
- 후속: coordinate missing 122건은 release blocker가 아니라 V1.1 데이터 품질 보강 후보로 둔다. 이번 기준 DB write/seed 수정은 하지 않았다.
- source/raw table anon read: RLS로 row 0건.
- Google Places/Photos runtime 재발: 확인되지 않음.
