...
Room: NURI-11-디자인시스템·접근성
Domain: NURI-11-디자인시스템·접근성
...
Ownership:
- screens: shared UI only; feature screens remain owning-room scope
- code/services/store: theme tokens, typography, spacing, common components and accessibility helpers
- Supabase: none
- tests/docs: typography/accessibility snapshots and component tests
- ownership boundary: `src/components`, theme, AppText, modal/input common UI and accessibility tokens

Open issue: common token consistency; no feature logic ownership
...
Bootstrap scope: read-only review first, then token-level changes explicitly approved by NURI-00.
Next dependency: activate only when a cross-feature token change is approved.
...
- Ownership doc: `docs/handoffs/2026-08-05/rooms/NURI-11-디자인시스템·접근성-ROOM-OWNERSHIP.md`
- Read canonical docs:
- Screens/code/service/store/Supabase/tests: shared UI only; feature screens remain owning-room scope; theme tokens, typography, spacing, common components and accessibility helpers; none; typography/accessibility snapshots and component tests
- Open issue: DESIGN-001 — common token consistency; no feature logic ownership
- Existing dirty changes: preserved and not staged
- Implementation status:
- Verification status:
- Release status:
- Boundaries: `src/components`, theme, AppText, modal/input common UI and accessibility tokens
