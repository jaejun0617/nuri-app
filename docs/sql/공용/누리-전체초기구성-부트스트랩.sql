...
-- ============================================================
-- 이 파일은 새 Supabase 프로젝트를 한 번에 부트스트랩할 때 참고하는 공용 전체 초기 구성본이다.
-- profiles/pets/memories/community 기본 테이블, auth trigger, storage bucket/policy까지 넓게 포함하지만 현재 운영 remote의 exact mirror는 아니다.
-- 도메인별 최신 상태 판단은 `docs/sql/<도메인명>/*-최종.sql`과 `supabase/migrations/*`를 우선하고, 이 파일은 bootstrap/release bundle 성격으로 유지한다.
-- linked remote와 내용이 어긋나면 이 파일만 단독 source of truth로 보지 말고 도메인 최종본과 migration history를 함께 확인해야 한다.
--
-- 목적:
