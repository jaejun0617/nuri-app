-- 이 파일은 `profiles.nickname_confirmed` 온보딩 계약을 관리하는 보조 SQL이다.
-- 닉네임 존재 여부와 별개로 설정 완료 여부를 추적하고, 신규 가입 기본값을 잠근다.
-- 단독 최종본이 아니라 `프로필-닉네임정책-현재로컬미러.sql`, `신규유저-안전닉네임처리-보조.sql`과 함께 인증/계정 최소 세트를 이룬다.
--
-- ============================================================
