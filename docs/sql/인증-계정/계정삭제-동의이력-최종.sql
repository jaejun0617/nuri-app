-- 파일: docs/sql/인증-계정/계정삭제-동의이력-최종.sql
-- 이 파일은 계정 동의 이력과 계정 삭제 기능을 운영 중인 프로젝트에 별도로 붙일 때 쓰는 공용 기능 추가 최종본이다.
-- `user_consent_history`, `account_deletion_requests`, `account_deletion_cleanup_items`, `delete_my_account()` 관련 객체를 관리한다.
-- 실행용 migration이 아니라 수동 release bundle이므로, linked remote 적용 여부와 현재 앱 계약을 먼저 확인한 뒤 사용해야 한다.
-- 전체 초기 구성 SQL과 일부 기본 테이블이 겹치더라도, 계정 삭제/동의 이력의 현재 기능 묶음은 이 파일을 기준으로 차이를 분석한다.
--
-- 목적:
-- - 약관/개인정보/마케팅 동의 이력을 저장한다.
-- - 계정 삭제를 "직접 삭제 + 익명화/비식별 보관 + 비동기 storage cleanup" 계약으로 승격한다.
-- - 삭제 요청 상태와 cleanup queue를 별도 테이블로 추적한다.
...
revoke all on function public.enqueue_account_deletion_cleanup_item(uuid, text, text) from public;
revoke all on function public.sync_account_deletion_cleanup_summary(uuid) from public;
revoke all on function public.create_account_deletion_request(text, text) from public;
revoke all on function public.mark_account_deletion_unknown(uuid) from public;
revoke all on function public.delete_my_account(uuid) from public;
revoke all on function public.claim_account_deletion_cleanup_items(integer) from public;
revoke all on function public.complete_account_deletion_cleanup_item(uuid, boolean, text, text) from public;
