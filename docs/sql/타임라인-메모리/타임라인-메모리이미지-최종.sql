-- 이 파일은 타임라인 메모리 이미지 시스템의 현재 최종본이다.
-- `memory_images` 테이블, thumbnail worker 상태 컬럼, owner 동기화 trigger, RLS와 read view를 관리한다.
-- 문서용 source of truth이며, linked remote 적용 전에는 `docs/sql/타임라인-메모리/메모리-이미지URL-호환성-추가.sql` 같은 보조 SQL과 충돌 여부를 먼저 확인해야 한다.
-- 실제 remote drift가 있으면 이 파일을 기준으로 차이를 분석하고, 실행 이력 자체는 migration history를 우선 본다.
