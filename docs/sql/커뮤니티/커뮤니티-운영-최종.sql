-- 이 파일은 커뮤니티 운영 스키마의 현재 최종본이다.
-- 게시글/댓글/좋아요/신고/모더레이션/이미지/콘텐츠 정책/조회수 관련 객체를 한 번에 보여주는 문서용 source of truth다.
-- 실행 기준은 `supabase/migrations/*`의 누적 이력과 함께 봐야 하며, linked remote drift가 있으면 이 파일을 기준으로 차이를 분석한다.
-- 신규 remote 적용 전에는 `docs/sql/커뮤니티/레거시/*` 분할본이 아니라 이 파일과 migration history를 먼저 확인해야 한다.
-- Task 6/Task 8 이후 기준으로 `community_blocked_terms`, `community_post_view_events`, `record_community_post_view()`까지 포함한다.
--
-- 작성일: 2026-03-23
-- 작성 기준:
...
-- - 게시/댓글/신고 rate limiting과 조회수 dedupe는 앱 임시 로직이 아니라 DB 계약을 source of truth로 본다.
...
on public.community_blocked_terms for select
using (public.is_community_admin());

create policy "community_blocked_term_patterns_select_admin"
on public.community_blocked_term_patterns for select
using (public.is_community_admin());
